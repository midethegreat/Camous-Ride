...
export interface WhatsAppResponse {
  ...
  restaurantResponse?: "1" | "2"; // 1 = accept, 2 = decline
  responseTime?: string;
}
...
class RiderApiService {
  ...
  // Twilio WhatsApp Integration for Restaurant Orders
  async sendOrderToRestaurant(order: WhatsAppOrder): Promise<WhatsAppResponse> {
    try {
      const response = await this.fetchWithAuth("/api/twilio/whatsapp/order", {
        method: "POST",
        body: JSON.stringify(order),
      });
      return response.data;
    } catch (error) {
      console.error("Failed to send order to restaurant via WhatsApp:", error);
      throw error;
    }
  }

  // Check for restaurant response via WhatsApp
  async checkRestaurantResponse(
    orderId: string,
  ): Promise<WhatsAppResponse | null> {
    try {
      const response = await this.fetchWithAuth(
        `/api/twilio/whatsapp/response/${orderId}`,
      );
      return response.data;
    } catch (error) {
      console.error("Failed to check restaurant response:", error);
      return null;
    }
  }

  // Handle restaurant response (1 = accept, 2 = decline)
  async handleRestaurantResponse(
    ...
    try {
      const result = await this.fetchWithAuth(
        "/api/twilio/whatsapp/handle-response",
        {
          method: "POST",
      ...
      console.error("Failed to handle restaurant response:", error);
      throw error;
    }
  }
}
...
// Utility function to format WhatsApp order message
export const formatWhatsAppOrderMessage = (order: WhatsAppOrder): string => {
  ...
  const message = `🍽️ *NEW FOOD ORDER* 🍽️
